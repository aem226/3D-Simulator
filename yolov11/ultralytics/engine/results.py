...
class Results(SimpleClass):
    ...
    def __init__(
        self, orig_img, path, names, boxes=None, masks=None, probs=None, keypoints=None, obb=None, speed=None, ang=None, dep=None
    ) -> None:
        """
        ...
        extra_fields = ['conf', 'cls', 'ang', 'dep']
        self.boxes = Boxes(boxes, self.orig_shape, extra_fields=extra_fields) if boxes is not None else None
        ...
        self.save_dir = None
         # Optional shortcuts
        self.ang = self.boxes.ang if self.boxes is not None else None
        self.dep = self.boxes.dep if self.boxes is not None else None
    ...
    def update(self, boxes=None, masks=None, probs=None, obb=None, keypoints=None):
        ...
        if boxes is not None:
            ...
            extra_fields = ['conf', 'cls', 'ang', 'dep']
            self.boxes = Boxes(ops.clip_boxes(boxes, self.orig_shape), self.orig_shape, extra_fields=extra_fields)
...
class Boxes(BaseTensor):
    """
    A class for managing and manipulating detection boxes with flexible extra fields.

    Supports xyxy, xywh, normalized coordinates, and optional fields like
    confidence, class, tracking ID, angle, depth, etc.

    Args:
        boxes (torch.Tensor | np.ndarray): Tensor/array with shape (N, 4 + extra_fields)
            First 4 columns must be [x1, y1, x2, y2].
        orig_shape (tuple[int, int]): Original image height and width.
        extra_fields (list[str], optional): Names of extra fields after xyxy.
            Example: ['conf', 'cls', 'id', 'ang', 'dep']
    """

    def __init__(self, boxes, orig_shape, extra_fields=None) -> None:
        if boxes.ndim == 1:
            boxes = boxes[None, :]
        n = boxes.shape[-1]
        assert n >= 6, f"expected at least 6 values but got {n}"  # xyxy, conf, cls, optional extras
        super().__init__(boxes, orig_shape)
        self.is_track = n == 7
        self.orig_shape = orig_shape
        self.extra_fields = extra_fields if extra_fields is not None else ['conf','cls']
    ...
    def xywh(self):
        ...
        w = x2 - x1
        h = y2 - y1
        return torch.stack([cx, cy, w, h], dim=-1) if isinstance(xy, torch.Tensor) else np.stack([cx, cy, w, h], axis=-1)

    @property
    @lru_cache(maxsize=2)
    def xyxyn(self):
        xy = self.xyxy.copy() if isinstance(self.data, np.ndarray) else self.xyxy.clone()
        xy[..., [0, 2]] /= self.orig_shape[1]
        xy[..., [1, 3]] /= self.orig_shape[0]
        return xy

    @property
    @lru_cache(maxsize=2)
    def xywhn(self):
        xywh = self.xywh.copy() if isinstance(self.data, np.ndarray) else self.xywh.clone()
        xywh[..., [0, 2]] /= self.orig_shape[1]
        xywh[..., [1, 3]] /= self.orig_shape[0]
        return xywh
    ...
    def _get_field(self, name):
        if name not in self.extra_fields:
            return None
        idx = self.extra_fields.index(name) + 4
        return self.data[:, idx]

    @property
    def conf(self): return self._get_field('conf')
    @property
    def cls(self): return self._get_field('cls')
    @property
    def id(self): return self._get_field('id') if self.is_track else None
    @property
    def ang(self): return self._get_field('ang')
    @property
    def dep(self): return self._get_field('dep')

    # ---------------- Device / format conversion ----------------
    def cpu(self):
        if self.data is not None and isinstance(self.data, torch.Tensor):
            new_data = self.data.cpu()
        else:
            new_data = self.data
        return Boxes(new_data, self.orig_shape, self.extra_fields)

    def cuda(self):
        if self.data is not None and isinstance(self.data, torch.Tensor):
            new_data = self.data.cuda()
        else:
            new_data = self.data
        return Boxes(new_data, self.orig_shape, self.extra_fields)
    ...
    def __getitem__(self, idx):
        return Boxes(self.data[idx], self.orig_shape, self.extra_fields)
