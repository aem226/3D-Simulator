...
class DetectionValidator(BaseValidator):
    ...
    def preprocess(self, batch):
        ...
        #lengths = [len(item) for item in batch["ang"]]
        #max_len = max(lengths)
        #batch["ang"] = [item + [0] * (max_len - len(item)) if len(item) < max_len else item[:max_len] for item in batch["ang"]]
        #batch["ang"] = torch.tensor(np.array(batch["ang"]))

        for k in ["batch_idx", "cls", "bboxes", "ang"]:
        ...
        if self.args.save_hybrid and self.args.task == "detect":
            ...
            self.lb = [
                torch.cat([batch["cls"][batch["batch_idx"] == i], bboxes[batch["batch_idx"] == i], batch["ang"][batch["batch_idx"] == i], batch["dep"][batch["batch_idx"] == i]], dim=-1)
                for i in range(nb)
            ]
    ...
    def _prepare_batch(self, si, batch):
        ...
        # if batch["ang"].device == 'cpu':
        #     batch["ang"] = batch["ang"].to(self.device)

        idx = batch["batch_idx"] == si
        #idx = (batch["batch_idx"] == si).to(self.device)  # <- force idx to match device
        print("\n\n DEBUGGING DEVICES \n\n")
        ...
        return {"cls": cls, "bbox": bbox, "ori_shape": ori_shape, "imgsz": imgsz, "ang": ang, "dep":dep, "ratio_pad": ratio_pad}
    ...
    def update_metrics(self, preds, batch):
        ...
        for si, pred in enumerate(preds):
            ...
            stat = dict(
                ...
                pred_ang = torch.zeros(0, device=self.device),
                pred_dep = torch.zeros(0, device=self.device),
                tp=torch.zeros(npr, self.niou, dtype=torch.bool, device=self.device),
            )
            pbatch = self._prepare_batch(si, batch)
            cls, bbox, ang, dep = pbatch.pop("cls"), pbatch.pop("bbox"), pbatch.pop("ang"), pbatch.pop("dep")
            nl = len(cls)
            stat["target_cls"] = cls
            ...
            stat["pred_ang"] = predn[:, 6]
            stat["pred_dep"] = predn[:, 7]
    ...
    def _process_batch(self, detections, gt_bboxes, gt_cls, gt_ang):
        """
        Return correct prediction matrix.

        Args:
            detections (torch.Tensor): Tensor of shape (N, 7) representing detections where each detection is
                (x1, y1, x2, y2, conf, class, angle).
            gt_bboxes (torch.Tensor): Tensor of shape (M, 4) representing ground-truth bounding box coordinates. Each
                bounding box is of the format: (x1, y1, x2, y2).
            gt_cls (torch.Tensor): Tensor of shape (M,) representing target class indices.
            gt_ang (torch.Tensor): Tensor of shape (M,) representing target angles.
        ...
        return self.match_predictions(detections[:, 5], gt_cls, detections[:, 6], gt_ang, detections[:, 7], gt_dep, iou)
