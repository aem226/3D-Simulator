...
def plot_images(
    ...
    ang: Optional[Union[torch.Tensor, np.ndarray]] = None,
    dep: Optional[Union[torch.Tensor, np.ndarray]] = None,
    ...
    for i in range(bs):
        ...
        if len(cls) > 0:
            ...
            angles = ang[idx] if ang is not None else None
            depths = dep[idx] if dep is not None else None
            ...
            if len(bboxes):
                ...
                for j, box in enumerate(boxes.astype(np.int64).tolist()):
                    ...
                    if labels or conf[j] > conf_thres:
                        angle_label = f" {angles[j]:.2f}" if angles is not None else ""
                        depth_label = f" {depths[j]:.2f}" if depths is not None else ""
                        label = f"{c}" if labels else f"{c} {conf[j]:.1f}{angle_label}{depth_label}"
...
def output_to_rotated_target(output, max_det=300):
    ...
    targets = []
    for i, o in enumerate(output):
        box, conf, cls, ang, dep = o[:max_det, :8].cpu().split((4, 1, 1, 1, 1), 1)
        j = torch.full((conf.shape[0], 1), i)
        targets.append(torch.cat((j, cls, ops.xyxy2xywh(box), conf, ang, dep), 1))#DOUBLE CHECK THE BOX PARAMETER
    targets = torch.cat(targets, 0).numpy()
    return targets[:, 0], targets[:, 1], targets[:, 2:-1], targets[:, -1]
