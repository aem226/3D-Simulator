        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                if any(len(x) > 6 for x in lb) and (not keypoint):  # is segment
                    ...
                    angle = np.array([x[5] for x in lb], dtype=np.float32) # (cls, xy1..., ang, dep)
                    depth = np.array([x[6] for x in lb], dtype=np.float32) # (cls, xy1..., ang, dep)
                    lb = np.concatenate((classes.reshape(-1, 1), segments2boxes(segments), angle.reshape(-1, 1), depth.reshape(-1, 1)), 1)  # (cls, xywh, ang)
                lb = np.array(lb, dtype=np.float32)
            nl = len(lb)
            if nl:
                if keypoint:
                    assert lb.shape[1] == (7 + nkpt * ndim), f"labels require {(7 + nkpt * ndim)} columns each"
                    points = lb[:, 7:].reshape(-1, ndim)[:, :2]
                else:
                ...
                ne = 1  # label empty
                lb = np.zeros((0, (7 + nkpt * ndim) if keypoint else 7), dtype=np.float32)
        else:
            nm = 1  # label missing
            lb = np.zeros((0, (7 + nkpt * ndim) if keypoints else 7), dtype=np.float32)
        if keypoint:
            keypoints = lb[:, 7:].reshape(-1, nkpt, ndim)
            if ndim == 2:
