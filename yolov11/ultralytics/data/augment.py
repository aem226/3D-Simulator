...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        ang = instances.angles
        dep = instances.depths
        ...
        new_instances = Instances(bboxes, ang, dep, segments, keypoints, bbox_format="xyxy", normalized=False)
        # Clip
...
class Format:
    ...
    def __call__(self, labels):
        ...
        labels["ang"] = torch.from_numpy(instances.angles) if nl else torch.zeros((nl, 0))
        labels["dep"] = torch.from_numpy(instances.depths) if nl else torch.zeros((nl, 0))
