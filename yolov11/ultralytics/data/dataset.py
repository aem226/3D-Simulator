...
class YOLODataset(BaseDataset):
    ...
    def get_labels(self):
        ...
        lengths = ((len(lb["cls"]), len(lb["bboxes"]), len(lb["ang"]), len(lb["segments"])) for lb in labels)
        len_cls, len_boxes, len_ang, len_segments = (sum(x) for x in zip(*lengths))
    ...
    def update_labels_info(self, label):
        ...
        angles = label.pop("ang")
        depths = label.pop("dep")
        ...
        label["instances"] = Instances(bboxes, angles, depths, segments, keypoints, bbox_format=bbox_format, normalized=normalized)
        return label
    ...
    def collate_fn(batch):
        ...
        for i, k in enumerate(keys):
            ...
            if k == "ang":
                # for j, tensor in enumerate(value):
                #     print(f'Tensor {j} device: {tensor.device}')
                value = [tensor.view(-1) if tensor.dim() > 1 else tensor for tensor in value]
                value = torch.cat(value).view(-1, 1)
            new_batch[k] = value
